...
7. 结束程序：
    ...
    - **推荐**：在终端按 `Ctrl + C`，程序会自动清除模拟定位后退出
    - 直接关闭终端窗口或执行 `kill <pid>` 也可安全退出（程序会捕获信号并完成清理）
    - **不要** 使用 `kill -9 <pid>`，该信号无法被捕获，会导致定位卡住

8. 如果定位卡住未恢复，运行以下命令恢复（无需重启手机）：

    ```shell
    python recover.py
    ```

    若 `recover.py` 也无法恢复，再考虑重启手机。
